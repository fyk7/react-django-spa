...
def _retry(func, retry_count, wait_time, **kwargs):
    if retry_count == 0:
        return None
    try:
        return func(**kwargs)
    ...
    return _retry(func, retry_count - 1, wait_time, **kwargs)


# TODO Add exception handling when css selectors no longer work.
def fetch_news():
    target_url = BLOOMBERG_URL
    res = _retry(requests.get, retry_count=3,
    ...
    for a in child_anchors:
        ...
        time.sleep(TIME_SLEEP)
        soup = BeautifulSoup(res.text, 'html.parser')
        news_dict['detail'] = ''.join([_prep_text(c.text) for c in soup.select(
